...
def visualize_shap_multiline(input_tokens, shap_values, original_value, masked_values, output_token, ax=None):
    ...
    for i, token in enumerate(input_tokens):
        ...
        ax.text(total_width, y_pos, token, fontsize=14, color="black",
                bbox=dict(facecolor=color, alpha=0.8, boxstyle="round,pad=0.3"))

        # Add SHAP differences below tokens
        ax.text(total_width + 0.1, y_pos - 0.5, f"{shap_difference:.2f}", fontsize=6, color="black", ha="center")
        ...
        total_width += text_width * 0.15 + 0.3  # Add extra spacing between tokens

        # Move to next line after 12 tokens
        # if i % tokens_per_line == 9:
        #     y_pos -= 1
        #     total_width = 0
        if total_width > 10:
            y_pos -= 1.2
    ...
    ax.set_xlim(0, 10)  # Limit X range to fit sentences
...
def render_shap_heatmap(summary_data, tokenizer):
    ...
    # ⬇️ Inject custom CSS to enlarge pills
    st.markdown("""
        <style>
    ...
    fig, ax = plt.subplots(figsize=(10, len(input_tokens) // 18))
